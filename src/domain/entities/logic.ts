...
export class Logic {
  ...
  static #whenClauseColumnRefsAreEqual = (
    fst: ColumnRef | undefined,
    snd: ColumnRef | undefined
  ): boolean => {
  ...
  static #assignWhenClauseDependencies = (
    ...
    columns.forEach((col, elementIndex) => {
      const prevCol = columns[elementIndex - 1];
      const thisCol = col;
      const nextCol = columns[elementIndex + 1];

      const isWhenClause = thisCol.context.path.includes(
        `${SQLElement.CASE_EXPRESSION}.${SQLElement.WHEN_CLAUSE}`
      );
      const singleWhenClause =
        isWhenClause &&
        !this.#whenClauseColumnRefsAreEqual(prevCol, thisCol) &&
        !this.#whenClauseColumnRefsAreEqual(thisCol, nextCol);
      const firstInWhenSequence =
        isWhenClause &&
        this.#whenClauseColumnRefsAreEqual(thisCol, nextCol) &&
        !(prevCol.dependencyType === DependencyType.QUERY);
  ...
  static #removePossibleDuplicateDependencies = (
    ...
    const uniqueDependencies = columns.filter(
      (col, elementIndex, self) =>
        elementIndex ===
        self.findIndex((testCol) =>
          this.#createdDependenciesAreEqual(testCol, col)
        )
    );
